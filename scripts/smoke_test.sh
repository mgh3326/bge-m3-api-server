...
DIM=$(echo "$HEALTH" | "$PYTHON_BIN" -c "import sys,json; print(json.load(sys.stdin)['embedding_dimension'])")
if [ "$DIM" != "1024" ]; then
...
curl -sf "$BASE_URL/v1/models" \
    -H "Authorization: Bearer $API_KEY" | "$PYTHON_BIN" -m json.tool
echo "PASS"
echo ""
...
EMB_DIM=$(echo "$RESP" | "$PYTHON_BIN" -c "import sys,json; print(len(json.load(sys.stdin)['data'][0]['embedding']))")
...
COUNT=$(echo "$RESP" | "$PYTHON_BIN" -c "import sys,json; print(len(json.load(sys.stdin)['data']))")
if [ "$COUNT" != "2" ]; then
